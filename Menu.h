...
class MenuItem {
public:
  byte screen;  // Index des écrans
  byte line;    // Numéro de ligne à l'écran
  const char *text;
...
public:
  const char *format;  // Format des valeurs lors de la saisie d'une variable
  T value = 0;
...
public:
  const char *format;  // Format des valeurs lors de la saisie d'une variable
  T *value;            // Pointeur vers l'adresse de la variable actuelle en cours de modification à l'écran
  T minVal;            // Limite la valeur d'une variable par le bas
  T maxVal;            // Limite la valeur d'une variable par le haut
  T increment;
...
public:
  ...
  byte index = 0;  // La variable stocke le numéro de la ligne du menu actuel
  ...
  void Draw(bool force = false) {
    ...
    if (page > 0)  // Affiche les flèches ⯅⯆ sur les lignes de menu correspondantes
      lcd.printAt(lcd.nCols - 1, 0, CH_UP);
    if (items[first + lcd.nRows]->screen == scr)
      lcd.printAt(lcd.nCols - 1, lcd.nRows - 1, CH_DW);
  }

  void Incactuel(int8_t increment, bool cycle = false) {
    byte cur = GetCursor();
  ...
  void DrawQuotes(bool enable) {
    ...
    lcd.printAt(leftPos, cur, enable ? CH_QR : ' ');   // Affichage du symbole >
    lcd.printAt(rightPos, cur, enable ? CH_QL : ' ');  // Affichage du symbole <
    lcd.printAt(0, cur, !enable ? CH_CR : ' ');        // Effacement du curseur principal
  }
...
private:
  ...
  byte GetTextCol() const {
    return (lcd.nCols > 16) ? 1 : 1;//espace entre le symbole fleche et le titre du menu (return (lcd.nCols > 16) ? 2 : 1;) = 1 espace
  }
  byte GetValueCol() const {
    return (lcd.nCols > 16) ? 13 : 10;//longueur nombre de caractere du menu
  }
  ...
  // pour le menu actuel on obtient l'index du premier élément
  byte GetFirstIndex() const {
    return index - items[index]->line;
  }

  // pour le menu courant on obtient l'index du dernier élément
